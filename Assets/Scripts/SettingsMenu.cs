...
public class SettingsMenu : MonoBehaviour
{
    ...
    [SerializeField]
    Dropdown graphicQualityDropdown;
    ...
    // Give the user preferences a default value if they don't yet exist
    void Start()
    {
    ...
    // Set the preferences values to the ones the user may have set in previous sessions
    void Update()
    {
        if (!runOnce)
        {
    ...
    void PopulateResolutions()
    {
        ...
        {
            // There is a refreshRate mismatch when using a game build (ie: 60Hz -> 59Hz), so it is reduced by 1 in these cases
            // Additionally only resolutions that match the user's screen refresh rate are added as to avoid duplicate entries
            float screenRefreshRate = Screen.currentResolution.refreshRate;

            if (
                resolution.refreshRate
                == (Application.isEditor ? screenRefreshRate : screenRefreshRate - 1)
            )
            {
        ...
        foreach (Resolution filteredResolution in filteredResolutions)
        {
            ...
            // If the resolution matches the user's screen, it is saved into our dropdown as it's starting value
            if (
                filteredResolution.width == Screen.width
